...
local wang_tile_short_side = 250
...
local probability_to_replace_pixelscene = occurence_rate_to_probability(wang_tile_short_side, wang_tile_count_h, wang_tile_count_v,
  custom_pixel_scene_count_h, custom_pixel_scene_count_v, config_altar_room_occurences_frozen_passages)

if probability_to_replace_pixelscene > 1 then
  print("AOD: We need more pixel scenes for frozen passages!")
  ...
  print("max_prob for frozen passages: " .. max_prob)
end
...
table.insert(g_pixel_scene_02, {
  prob   		    	= probability,
  material_file 	= "mods/anvil_of_destiny/files/loader_scenes/biome-plus/frozen_passages.png",
  visual_file		  = "",
  background_file	= "",
  is_unique		    = 0
})
